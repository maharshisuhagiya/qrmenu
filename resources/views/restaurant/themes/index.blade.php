                                                                ...
                                                                <div class="row mt-3">
                                                                    <div class="col-md-12">
                                                                        @if (strtolower($theme['name']) == strtolower($restaurant->theme ?? current($themes)['name']))
                                                                            <span class="btn btn-sm btn-success disabled mb-md-2 w-100">{{ __('system.crud.default') }}</span>
                                                                        @else
                                                                            {!! Form::open(['method' => 'put', 'route' => ['restaurant.themes.update']]) !!}
                                                                            <input type="hidden" name="theme" value="{{ $theme['name'] }}">
                                                                            <button type="submit" class="btn btn-sm btn-primary mb-md-2 w-100">{{ __('system.crud.active') }}</button>
                                                                            {!! Form::close() !!}
                                                                        @endif
